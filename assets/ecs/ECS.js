...
/**
 * <ECS>是一个ECS系统的实例----又名"实体管理器"
 * 管理--->1: 组件<Component>, 2: 系统<System>, 3: 集合<Group>, 4: 监听器<Observer>, 5: 处理器<Handler>的注册
 * 维护实体<Entity>的对象池,负责集合<Group>,组件池<ComponentPool>,
 * 组件单例<ComponentSingleton>的创建
 * 负责处理帧更新和事件传递
 *
 * @param {Number} updateTime  33.333--暂时不能为0
 * @param {Number} timeScale 1
 * @param {Object} opt {"server":true}
 * @constructor
 */
class ECS {

    constructor(updateTime, timeScale, opt) {
        cc.log('wcx updateTime=' + updateTime + ' timeScale=' + timeScale + ' opt=' + JSON.stringify(opt))
        opt = opt || {};
